...
targets, the encoder, the watermark and the card's drawing version. The
narration and the words reach here through the cascade (rule 2), so
they need no key. No story checkpoint predates this build, so the
compares are strict; `story: True` is the shape marker that keeps a clip
checkpoint from ever serving a story job or the reverse.
"""
...
class StoryRenderStage(Stage):
    ...
    def run(self, ctx: StageContext) -> dict:
        ...
        card_styles, card_events = story_card.overlay(preset, title, title_end)
        ...
        mark = watermark.compose(
            watermark.resolve(ctx.settings, say=lambda m: ctx.emit(-1, m)),
            renderer.OUT_W, renderer.OUT_H, preset, duration, say=lambda m: ctx.emit(-1, m),
        )
